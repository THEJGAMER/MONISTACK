...
- [x] **Full-text search across saved results** - 2026-08-12. Search
      ...
      **The text-search config is load-bearing, not a default.** Verified
      ...
      deployed app.
- [x] API tokens + a documented public API. **Done 2026-09-15.**
      `/api/docs` (Swagger) and `/api/redoc`, versioned 1.0 with a description
      of auth, roles and the versioning promise. Three ways in, one identity:
      the session cookie, `Authorization: Bearer sb_…` API tokens (created on
      Settings; hashed at rest, shown once, role-capped at the creator's,
      expiring, revocable), and Keycloak access tokens validated against the
      realm's JWKS with the same client-role mapping as interactive login.
      Live-verified: a viewer token reads, gets 403 on admin routes, and 401s
      the moment it is revoked.
- [x] Webhooks for external integration. **Done 2026-09-15.**
      Register a URL with an event filter; deliveries are JSON, HMAC-SHA256
      signed (`X-Switchboard-Signature`), retried 3x with backoff on transient
      failures and not on 4xx, with the last status and consecutive failures on
      the row - never silently disabled. Live-verified against a local
      listener: the signature validates with the once-shown secret. Every emit
      goes through an in-process bus (`events.py`) shared with paging.
- [x] Mobile/responsive pass (the front panel especially). **Done 2026-09-15.**
      The app is a PWA: manifest, icons, a service worker served at root scope,
      an install prompt. Paging is in-house via Web Push (adapted from PROXMON's
      push.ts/sw.js, extended): per-device severity floor, resolve notifications,
      requireInteraction on critical, a per-alarm tag so re-fires replace rather
      than stack, and an **Acknowledge action on the notification** that acks
      with the browser's own session from the lock screen. The chassis art
      scrolls sideways on phones instead of squashing. Topology's primary view
      is now tables (phone-usable) with the diagram one tab over.
- [x] Per-device notes/runbook links. **Done 2026-09-15.**
      Two fields on the device form, stored on the device record, shown on the
      Console page's device summary with the runbook as a link.
